        ...
        foreach ($positionsArray as $position) {
            ...
            if (substr($position, 0, 5) == 'hook:') {
                ...
                $templates = $db->fetchAll("
					...
					WHERE template_compiled LIKE " . XenForo_Db::quoteLike('callTemplateHook(\'' . substr($position,
                            5) . '\',', 'lr') . "
				");
                if (count($templates) > 0) {
                    ...
                    $dw->error(new XenForo_Phrase('wf_non_existent_hook_x', array('hook' => substr($position, 5))),
                        $fieldName);
                    return false;
                }
            } else {
