    ...
    public static function template_post_render(
        ...
        if (defined('WIDGET_FRAMEWORK_LOADED')) {
            if ($templateName != 'wf_widget_wrapper') {
                WidgetFramework_Core::getInstance()->renderWidgetsFor($templateName, $template->getParams(), $template,
                    $containerData);
            ...
            if ($templateName === 'PAGE_CONTAINER') {
                WidgetFramework_Template_Extended::WidgetFramework_processLateExtraData($output, $containerData,
                    $template);
