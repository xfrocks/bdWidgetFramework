...
class WidgetFramework_Core
{
    ...
    public function renderWidgetsFor(
        ...
        $html = $this->_renderWidgetsFor($templateName,
            array_merge($params, array(WidgetFramework_WidgetRenderer::PARAM_POSITION_CODE => $templateName)),
        ...
        if ($html != $originalHtml) {
            ...
            if (!empty($containerData['sidebar']) AND self::debugMode()) {
                $containerData['sidebar'] .= sprintf('<div>Widget Framework is in debug mode<br/>Renderers: %d<br/>Widgets: %d<br/></div>',
    ...
    public static function getRenderer($class, $throw = true)
    {
        ...
        try {
            if (in_array($class, $instance->_renderers)) {
                ...
                if ($throw) {
                    throw new XenForo_Exception(new XenForo_Phrase('wf_invalid_widget_renderer_x',
                        array('renderer' => $class)), true);
                }
            }
        } catch (Exception $e) {
            if ($throw) {
