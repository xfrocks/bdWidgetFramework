...
class WidgetFramework_WidgetRenderer_RecentStatus extends WidgetFramework_WidgetRenderer_ProfilePosts
{
    ...
    protected function _getProfilePosts(
        ...
        if (XenForo_Visitor::getUserId() == 0 OR empty($widget['options']['friends_only'])) {
            ...
            $users = $userModel->getFollowedUserProfiles(XenForo_Visitor::getUserId(), $widget['options']['limit'],
        ...
        if (!empty($widget['options']['show_duplicates']) AND !empty($users)) {
            ...
            $profilePostModel = $core->getModelFromCache('XenForo_Model_ProfilePost');
            $profilePostIds = $profilePostModel->WidgetFramework_getProfilePostIdsOfUserStatuses($userIds,
        ...
        $profilePosts = $profilePostModel->getProfilePostsByIds($profilePostIds,
            array('join' => XenForo_Model_ProfilePost::FETCH_USER_POSTER | XenForo_Model_ProfilePost::FETCH_USER_RECEIVER | XenForo_Model_ProfilePost::FETCH_USER_RECEIVER_PRIVACY));
