...
class WidgetFramework_Template_Helper_Layout
{
    ...
    public static function getContainerSize(array $widgets, $width = 200, $height = 200, $gap = 10)
    {
        ...
        return sprintf('width: %dpx; height: %dpx;', $maxCol * ($width + $gap) - $gap,
            $maxRow * ($height + $gap) - $gap);
    }
    ...
    /**
     * @param array $widgetPageOptions
     * @param array $widgets
     * @param _Layout_Vertical $layoutTree
     * @return string
     */
    public static function generateCss(array $widgetPageOptions, array $widgets, $layoutTree)
    {
        ...
        foreach ($cssClasses['cols'] as $cssClassCol) {
            ...
            if (!empty($widgetPageOptions['column_gap'])) {
                $css[] = sprintf('.%s > .margin { margin-right: %dpx; }', $cssClassCol['name'],
                    $widgetPageOptions['column_gap']);
            }
            ...
            if (!empty($cssClassCol['width'])) {
                ...
                foreach ($cssClasses['xOfY'] as $cssClassXOfY) {
                    if ($cssClassXOfY['cssClassLayoutCols'] == $cssClassCol['name']) {
                        self::generateCssMedia($cssClassCol['width'], 0, $hasSidebar, $cssMedia, array(
                            sprintf('.%s > .%s { width: %f%%; }', $cssClassCol['name'], $cssClassXOfY['name'],
                                $cssClassXOfY['widthPercent'])
                        ));
                    } elseif ($cssClassCol['width'] < $cssClassXOfY['cssClassLayoutColsWidth']) {
                        self::generateCssMedia($cssClassCol['width'], $cssClassXOfY['cssClassLayoutColsWidth'],
                            $hasSidebar, $cssMedia, array(
                                sprintf('.%s > .%s.%s { width: 100%%; }', $cssClassXOfY['cssClassLayoutCols'],
                                    $cssClassCol['name'], $cssClassXOfY['name']),
                                sprintf('.%s > .%s.%s > .margin { margin-right: 0; }',
                                    $cssClassXOfY['cssClassLayoutCols'], $cssClassCol['name'], $cssClassXOfY['name']),
                            ));
                    }
                }
            }
    ...
    public static function generateCssMedia($minWidth, $maxWidth, $hasSidebar, array &$cssMedia, array $rules)
    {
        ...
        if ($minWidth === 0) {
            ...
            foreach ($ranges as $range) {
                if ($range[0] > 0) {
                    $mediaStatements[] = sprintf('screen and (min-width: %dpx) and (max-width: %dpx)', $range[0],
                        $range[1]);
                } else {
