...
class WidgetFramework_Template_Helper_Layout {
	...
	public static function getWidgetPositionAndSize(array $widget, $width = 200, $height = 200, $gap = 10) {
		...
		if (isset($widget['options']['layout_row'])) $row = max($row, $widget['options']['layout_row']);
		if (isset($widget['options']['layout_col'])) $col = max($col, $widget['options']['layout_col']);
		if (isset($widget['options']['layout_sizeRow'])) $sizeRow = max($sizeRow, $widget['options']['layout_sizeRow']);
		if (isset($widget['options']['layout_sizeCol'])) $sizeCol = max($sizeCol, $widget['options']['layout_sizeCol']);

		return sprintf('width: %dpx; height: %dpx; top: %dpx; left: %dpx;',
