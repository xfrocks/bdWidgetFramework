	...
	public function actionSave()
	{
		...
		foreach ($widgets as $widget) {
			...
			if (!empty($widgetsInput[$widget['widget_id']])) {
				...
				if (isset($inputRef['layout_row'])
				AND (!isset($widget['options']['layout_row'])
						OR ($inputRef['layout_row'] != $widget['options']['layout_row']))) {
					$changed = true;
					$newOptions['layout_row'] = $inputRef['layout_row'];
				}
				if (isset($inputRef['layout_col'])
				AND (!isset($widget['options']['layout_col'])
						OR ($inputRef['layout_col'] != $widget['options']['layout_col']))) {
					$changed = true;
					$newOptions['layout_col'] = $inputRef['layout_col'];
				}
				if (isset($inputRef['layout_sizeRow'])
				AND (!isset($widget['options']['layout_sizeRow'])
						OR ($inputRef['layout_sizeRow'] != $widget['options']['layout_sizeRow']))) {
					$changed = true;
					$newOptions['layout_sizeRow'] = $inputRef['layout_sizeRow'];
				}
				if (isset($inputRef['layout_sizeCol'])
				AND (!isset($widget['options']['layout_sizeCol'])
						OR ($inputRef['layout_sizeCol'] != $widget['options']['layout_sizeCol']))) {
					$changed = true;
