...
class WidgetFramework_ControllerAdmin_Widget extends XenForo_ControllerAdmin_Abstract
{
    ...
    {
        return $this->_getToggleResponse($this->_getWidgetModel()->getGlobalWidgets(false),
            'WidgetFramework_DataWriter_Widget', 'widgets');
    }
    ...
    protected function _getWidgetOrError($widgetId)
    {
        ...
        if (!$info) {
            throw $this->responseException($this->responseError(new XenForo_Phrase('wf_requested_widget_not_found'),
                404));
        }
